...
//import KIP17ABI from '../abi/MintKIP17ABI.json';
...
const option = {
  ...
  export const fetchCardsOf = async (address) => {
    ...
    for(let i=0;i<balance;i++){
      ...
      tokenPosXs.push(uriJSON.posX);
      tokenPosXs.push(uriJSON.posY);
    }

    const nfts = [];
    for(let i=0;i<balance;i++){
      nfts.push({ id:tokenIds[i], uri:tokenUris[i], posX: tokenPosXs[i], posX: tokenPosYs[i] })
    }
    console.log(nfts);
    return nfts;
  };
